...
interface MaterialDao {
    ...
    @Query("SELECT * FROM materials WHERE mIsDeleted = 0 ORDER BY mName")
    fun getMaterialsList(): LiveData<List<Material>>

    @Query("SELECT * FROM materials WHERE mIsDeleted = 0 ORDER BY mName")
    suspend fun getMaterialsListSync(): List<Material>

    @Query("SELECT * FROM workOrderHistoryMaterials WHERE workOrderHistoryMaterialId = :id")
    suspend fun getWorkOrderHistoryMaterialSync(id: Long): WorkOrderHistoryMaterial?

    @RewriteQueriesToDropUnusedColumns
    @Transaction
    @Query("SELECT * FROM materialMerged WHERE mmMasterId = :materialId AND mmIsDeleted = 0")
    fun getMaterialAndChildList(materialId: Long): LiveData<List<MaterialAndChild>>

    @Query("SELECT * FROM materials WHERE mName LIKE :query AND mIsDeleted = 0 ORDER BY mName")
    fun searchMaterials(query: String): LiveData<List<Material>>

    @Query("SELECT * FROM materialMerged WHERE materialMergeId = :id")
    suspend fun getMaterialMergedSync(id: Long): MaterialMerged?

    @Query("SELECT * FROM materials WHERE materialId = :materialId AND mIsDeleted = 0")
    fun getMaterial(materialId: Long): LiveData<Material>

    @Query("SELECT * FROM materials WHERE materialId = :materialId AND mIsDeleted = 0")
    suspend fun getMaterialSync(materialId: Long): Material?

    @Query("SELECT * FROM materials WHERE mName = :mName AND mIsDeleted = 0")
    ...
    @Query("UPDATE materialMerged SET mmIsDeleted = 1, mmUpdateTime = :updateTime WHERE materialMergeId = :materialMergedId")
    suspend fun deleteMaterialMerged(materialMergedId: Long, updateTime: String)

    @Query("UPDATE materials SET mIsDeleted = 1, mUpdateTime = :updateTime WHERE materialId = :materialId")
    ...
    @Query("UPDATE workOrderHistoryMaterials SET wohmIsDeleted = 1, wohmUpdateTime = :updateTime WHERE wohmHistoryId = :historyId")
    ...
    @Query("UPDATE workOrderHistoryMaterials SET wohmIsDeleted = 1, wohmUpdateTime = :updateTime WHERE workOrderHistoryMaterialId = :historyMaterialId")
    suspend fun deleteWorkOrderHistoryMaterial(historyMaterialId: Long, updateTime: String)

    @RewriteQueriesToDropUnusedColumns
    @Transaction
    @Query("SELECT COALESCE(mmMasterId, wohmMaterialId) as wohmMaterialId, wohmHistoryId, null as workOrderHistoryMaterialId, SUM(wohmQuantity) as wohmQuantity, MIN(wohmSequence) as wohmSequence, 0 as wohmIsDeleted, MAX(wohmUpdateTime) as wohmUpdateTime FROM workOrderHistoryMaterials LEFT JOIN materialMerged ON wohmMaterialId = mmChildId AND mmIsDeleted = 0 WHERE wohmHistoryId = :historyId AND wohmIsDeleted = 0 GROUP BY COALESCE(mmMasterId, wohmMaterialId) ORDER BY wohmUpdateTime")
    fun getMaterialsByHistoryCombined(historyId: Long): LiveData<List<WorkOrderHistoryMaterialCombined>>

    @RewriteQueriesToDropUnusedColumns
    @Transaction
    @Query("SELECT * FROM workOrderHistoryMaterials WHERE wohmHistoryId = :historyId AND wohmIsDeleted = 0 ORDER BY wohmUpdateTime")
    fun getMaterialsByHistory(historyId: Long): LiveData<List<WorkOrderHistoryMaterialCombined>>

    @RewriteQueriesToDropUnusedColumns
    @Transaction
    @Query("SELECT * FROM workOrderHistory INNER JOIN (SELECT * FROM workOrderHistoryMaterials WHERE workOrderHistoryMaterialId = :woHistoryMaterialId AND wohmIsDeleted = 0) ON woHistoryId = wohmHistoryId WHERE woHistoryDeleted = 0")
