...
fun WorkExtraTypeScreen(
    ...
    var wetName by rememberSaveable { mutableStateOf(initialExtraType?.wetName ?: "") }
    var wetAppliesTo by rememberSaveable {
        mutableIntStateOf(
            initialExtraType?.wetAppliesTo ?: ExtraAppliesToFrequencies.HOURLY.value
        )
    }
    var wetAttachTo by rememberSaveable {
        mutableIntStateOf(
            initialExtraType?.wetAttachTo ?: ExtraAttachToFrequencies.HOURLY.value
        )
    }
    var wetIsCredit by rememberSaveable { mutableStateOf(initialExtraType?.wetIsCredit ?: false) }
    var wetIsDefault by rememberSaveable { mutableStateOf(initialExtraType?.wetIsDefault ?: false) }
