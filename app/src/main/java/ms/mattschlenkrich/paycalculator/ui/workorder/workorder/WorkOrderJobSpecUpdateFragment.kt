...
class WorkOrderJobSpecUpdateFragment :
    ...
    private suspend fun populateJobSpecValues() =
        withContext(Dispatchers.Main) {
            if (mainActivity.mainViewModel.getWorkOrderJobSpecId() != null) {
                mainActivity.workOrderViewModel.getWorkOrderJobSpec(
                    mainActivity.mainViewModel.getWorkOrderJobSpecId()!!
    ...
    private suspend fun populateWorkOrderInfo() =
        withContext(Dispatchers.Main) {
            if (mainActivity.mainViewModel.getWorkOrder() != null) {
                workOrder = mainActivity.mainViewModel.getWorkOrder()!!
    ...
    private fun insertJobSpecIntoDb(jobSpecName: String): Long {
        ...
        mainActivity.workOrderViewModel.insertJobSpec(newJobSpec)
    ...
    private fun updateWorkOrderJobSpec(jobSpecId: Long, areaId: Long?) {
        CoroutineScope(Dispatchers.Main).launch {
            ...
            delay(WAIT_250)
            mainActivity.workOrderViewModel.updateWorkOrderJobSpec(
                WorkOrderJobSpec(
                    originalJobSpec.workOrderJobSpec.workOrderJobSpecId,
                    originalJobSpec.workOrderJobSpec.wojsWorkOrderId,
                    jobSpecId,
                    areaId,
                    note,
                    originalJobSpec.workOrderJobSpec.wojsSequence,
                    false,
                    df.getCurrentTimeAsString()
                )
            )
